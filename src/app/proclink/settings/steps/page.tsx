...
export default function WorkSettingsPage() {
  ...
  // -----------------------------
  const [codesMap, setCodesMap] = useState<Map<string, WorkTypeCodeDoc>>(new Map());
  ...
  // -----------------------------
  const [templatesMeta, setTemplatesMeta] = useState<Map<string, TemplateMeta>>(new Map());
  ...
  const workTypes: WorkTypeRow[] = useMemo(() => {
    ...
    ids.forEach((id) => {
      ...
      const source: WorkTypeRow["source"] = c && t ? "both" : c ? "codes" : "templates";
  ...
  const maxOrder = useMemo(() => {
    const orders = workTypes.map((x) => x.order).filter((n) => Number.isFinite(n));
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const baseTitle = String(data.title ?? selectedWork?.label ?? workTypeId);
        setTitle(baseTitle);
  ...
  const onAddStep = () => {
    setSteps((prev) =>
      renumberByArrayOrder([...prev, { id: uid(), name: "", order: prev.length + 1 }]),
    );
  };
  ...
  const onSaveTemplate = async () => {
    ...
    const fixedTitle = String(title || selectedWork?.label || workTypeId).trim();
    ...
    <div className="mx-auto max-w-5xl px-4 py-6 text-gray-900 dark:text-gray-100">
      ...
      <div className="mt-6 rounded-2xl border bg-white p-4 dark:bg-gray-950 dark:border-gray-800">
        <div className="grid gap-4 md:grid-cols-3 md:items-end">
          <div className="grid gap-1">
            ...
            <div className="text-[11px] text-gray-500 dark:text-gray-400 break-all">
              選択ID: <span className="font-mono">{workTypeId}</span>
            </div>
          </div>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <div className="rounded-xl border p-3 dark:border-gray-800">
            <div className="text-sm font-semibold">工事種類を追加（documentIDは自動）</div>
            ...
            <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
              ...
              <span className="font-mono">{COL_TEMPLATES}</span> の両方を作成します。
            </div>
          </div>
          ...
          <div className="rounded-xl border p-3 dark:border-gray-800">
            ...
            <p className="mt-1 text-xs text-gray-600 dark:text-gray-300">
              ...
              <span className="font-mono"> {COL_CODES}</span> 未登録のものを作成します（IDは既存テンプレIDを維持）。
            </p>
      ...
      <div className="mt-6 rounded-2xl border bg-white p-4 dark:bg-gray-950 dark:border-gray-800">
        ...
        <div className="mt-4 rounded-xl border p-3 dark:border-gray-800">
          ...
          {loading ? (
            <div className="mt-3 text-sm text-gray-600 dark:text-gray-300">読み込み中...</div>
          ) : steps.length === 0 ? (
